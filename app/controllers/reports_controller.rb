class ReportsController < ApplicationController
  def index
    @start_date = params[:start_date].presence || Date.current.beginning_of_month.to_s
    @end_date   = params[:end_date].presence || Date.current.end_of_month.to_s
    ...
    if params[:entry_type].present?
      @entries = @entries.joins(:entry_type)
                         .where(entry_types: { name: params[:entry_type] })
    end

    if params[:entry_status].present?
      @entries = @entries.joins(:entry_status)
                         .where(entry_statuses: { name: params[:entry_status] })
    end

    @entries = @entries.order(occurred_on: :desc, created_at: :desc)
...
@balance = @income_total - @expense_total
  ...
  def pdf
    ...
    # saldo real do período: só pagos
    paid_entries = current_user.entries
                               .joins(:entry_status)
                               .where(entry_statuses: { name: "paid" })
                               .where(occurred_on: @start_date..@end_date)
  ...
  def generate_pdf
    ...
    pdf.fill_color "475569"
    pdf.text "Período: #{Date.parse(@start_date.to_s).strftime('%d/%m/%Y')} até #{Date.parse(@end_date.to_s).strftime('%d/%m/%Y')}", size: 10
    pdf.move_down 3
    ...
    pdf.text "Tipo: #{tipo_label}", size: 10
    pdf.text "Status: #{status_label}", size: 10
    ...
    pdf.table(summary_data,
      ...
      columns(2).text_color = @balance.to_f >= 0 ? "16A34A" : "DC2626"
    end
